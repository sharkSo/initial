...
public class AnalyticCtrl {
	...
	private static final String TOTAL_LIKES_BY_DAY = "SELECT COUNT(*),MONTH(TO_TIMESTAMP(DATE_INPUT)),YEAR(TO_TIMESTAMP(DATE_INPUT)) FROM FACEBOOK_FEED_DATA_LIKES GROUP BY MONTH(TO_TIMESTAMP(DATE_INPUT)),YEAR(TO_TIMESTAMP(DATE_INPUT))";
